"""
Load Article 15(1)(d) data (own-initiative moderation of ToS violations).

Handles with-or-without Category label, header whitespace, comma-formatted
numerics, merged-cell forward-fill, self-healing FK, and provider variations
in the optional 'Description of the sub-category Other' column name.
"""
import os
import sys
import json
...
CSV_NAME = "06_A151D_241AB_Own_TC.csv"
PERIOD_COL = "Reporting period"
PLACEHOLDER = "YYYY-MM-DD/YYYY-MM-DD"
OPTIONAL_LABEL_COL = "Category label"

# Some providers (e.g. X) name the description column slightly differently.
DESC_OTHER_VARIANTS = [
...
REQUIRED_COLUMN_MAP = {
    "Category of incompatibility with the provider's terms and conditions": "category_code",
...
def main():
    ...
    # Some providers ship CSVs in Latin-1/Windows-1252 instead of UTF-8 (e.g. Amazon's
    # restated file 6 carries a note with smart quotes). Try encodings in order.
    df_raw = None
    ...
    with engine.begin() as conn:
        conn.execute(
            text("DELETE FROM own_initiative_tc WHERE filing_id = :fid"),
            {"fid": filing_id}
        )
        conn.execute(text("""
        ...
        if len(real_rows) == 0:
            log_anomaly(
                ...
                description=(
                    ...
                    f"placeholder reporting period '{PLACEHOLDER}'. No data loaded."
                ),
            )
...
# Some providers capitalise column headers differently. Match
    ...
    # Resolve the optional description_other column from a list of known variants
    desc_col = next((c for c in DESC_OTHER_VARIANTS if c in real_rows.columns), None)
    if desc_col:
        print(f"Using '{desc_col}' for description_other")
    else:
        print(f"No description_other column found; setting to NULL")

    cat_col = "Category of incompatibility with the provider's terms and conditions"
    ...
    # Add description_other column (may be from CSV or NULL)
    df["description_other"] = real_rows[desc_col].values if desc_col else None
    ...
    df.to_sql("own_initiative_tc", engine, if_exists="append", index=False)
